...
class VM_Version_StubGenerator: public StubCodeGenerator {
 public:
  ...
  address generate_detect_misaligned_vector(address* fault_pc1, address* fault_pc2, address* continuation_pc, int ebytes) {
    StubCodeMark mark(this, "VM_Version", "detect_misaligned_vector_stub");
#   define __ _masm->
    address start = __ pc();
    Assembler::SEW sew = Assembler::elembytes_to_sew(ebytes);

    __ enter();
    __ mv(x10, zr);
    __ la(t1, ExternalAddress((address) array));
    __ addi(t1, t1, ebytes - 1);   // Misaligned address
...
bool VM_Version::detect_misaligned_vector_support() {
  ...
  return detect_misaligned_vector_support_ebytes(2) &&
         detect_misaligned_vector_support_ebytes(4) &&
         detect_misaligned_vector_support_ebytes(8);
}

bool VM_Version::detect_misaligned_vector_support_ebytes(int ebytes) {
  ResourceMark rm;
  ...
  detect_misaligned_vector_stub = CAST_TO_FN_PTR(detect_misaligned_vector_stub_t,
                                                 ...
                                                 &VM_Version::_misaligned_vector_continuation_pc, ebytes));
