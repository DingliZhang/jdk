...
#define INSN(NAME, op, funct3)                                                                     \
  void NAME(Register Rd, Register Rs, const int32_t offset) {                                      \
    guarantee(is_simm(offset, 12), "offset is invalid.");                                          \
    unsigned insn = 0;                                                                             \
...
#define INSN(NAME, op, funct3)                                                                           \
  void NAME(Register Rs1, Register Rs2, const int64_t offset) {                                          \
    guarantee(is_simm(offset, 13) && ((offset % 2) == 0), "offset is invalid.");                       \
    unsigned insn = 0;                                                                                   \
...
#define INSN(NAME, op, funct3)                                                        \
  void NAME(Register Rd, const uint32_t csr, const uint32_t uimm) {                   \
    guarantee(is_uimm12(csr), "csr is invalid");                                      \
    guarantee(is_uimm(uimm, 5), "uimm is invalid");                                   \
    unsigned insn = 0;                                                                \
...
#define INSN(NAME, op)                                                                \
  void NAME(Register Rd, const int32_t offset) {                                      \
    guarantee(is_simm(offset, 21) && ((offset % 2) == 0), "offset is invalid.");    \
    unsigned insn = 0;                                                                \
    patch((address)&insn, 6, 0, op);                                                  \
    patch_reg((address)&insn, 7, Rd);                                                 \
...
#define INSN(NAME, op, funct3)                                                              \
  void NAME(Register Rd, Register Rs1, int32_t imm) {                                       \
    guarantee(is_simm(imm, 12), "Immediate is out of validity");                            \
    unsigned insn = 0;                                                                      \
    patch((address)&insn, 6, 0, op);                                                        \
    patch((address)&insn, 14, 12, funct3);                                                  \
...
#define INSN(NAME, op, funct3)                                            \
  void NAME(Register Rd, uint32_t imm, SEW sew, LMUL lmul = m1,           \
            VMA vma = mu, VTA vta = tu, bool vill = false) {              \
    unsigned insn = 0;                                                    \
    guarantee(is_uimm(imm, 5), "imm is invalid");                         \
    patch((address)&insn, 6, 0, op);                                      \
    patch((address)&insn, 14, 12, funct3);                                \
...
#define INSN(NAME, op, funct3, funct6)                                                             \
  void NAME(VectorRegister Vd, VectorRegister Vs2, int32_t imm, VectorMask vm = unmasked) {        \
    guarantee(is_simm(imm, 5), "imm is invalid");                                                  \
    patch_VArith(op, Vd, funct3, (uint32_t)(imm & 0x1f), Vs2, vm, funct6);                         \
  }
...
#define INSN(NAME, op, lumop, vm, mop, nf)                                           \
  void NAME(VectorRegister Vd, Register Rs1, uint32_t width = 0, bool mew = false) { \
    guarantee(is_uimm(width, 3), "width is invalid");                                \
...
#define INSN(NAME, funct3, op)                                                               \
  void NAME(int32_t offset) {                                                                \
    assert_cond(is_simm(offset, 12) && ((offset % 2) == 0));                               \
    uint16_t insn = 0;                                                                       \
    c_patch((address)&insn, 1, 0, op);                                                       \
  ...
  void NAME(address dest) {                                                                  \
    ...
    assert_cond(is_simm(distance, 12) && ((distance % 2) == 0));                           \
    c_j(distance);                                                                           \
  }                                                                                          \
  void NAME(Label &L) {                                                                      \
...
#define INSN(NAME, funct3, op)                                                               \
  void NAME(Register Rs1, int32_t imm) {                                                     \
    assert_cond(is_simm(imm, 9) && ((imm % 2) == 0));                                      \
    uint16_t insn = 0;                                                                       \
    c_patch((address)&insn, 1, 0, op);                                                       \
    c_patch((address)&insn, 2, 2, (imm & nth_bit(5)) >> 5);                                  \
  ...
  void NAME(Register Rs1, address dest) {                                                    \
    assert_cond(dest != NULL);                                                               \
    int64_t distance = dest - pc();                                                          \
    assert_cond(is_simm(distance, 9) && ((offset % 2) == 0));                              \
    NAME(Rs1, distance);                                                                     \
  }                                                                                          \
    ...
    if (do_compress()) {                                                                     \
      if (Rd == Rs1 && is_simm(imm, 6)) {                                                    \
        ...
        if (Rd == Rs1 && (imm & 0b1111) == 0x0 && is_simm10(imm)) {                          \
          c_addi16sp(imm);                                                                   \
          return;                                                                            \
        } else if (Rd->is_compressed_valid() && (imm & 0b11) == 0x0 && (imm >= 0) && is_uimm(imm, 10)) { \
          c_addi4spn(Rd, imm);                                                               \
          return;                                                                            \
        }                                                                                    \
      }                                                                                      \
...
#define INSN(NAME)                                                                           \
  void NAME(Register Rd, Register Rs1, int32_t imm) {                                        \
    /* addiw -> c.addiw */                                                                   \
    if (do_compress() && (Rd == Rs1 && Rd != x0 && is_simm(imm, 6))) {                       \
      c_addiw(Rd, imm);                                                                      \
      return;                                                                                \
    }                                                                                        \
    _addiw(Rd, Rs1, imm);                                                                    \
...
#define INSN(NAME)                                                                           \
  void NAME(Register Rd, Register Rs1, int32_t imm) {                                        \
    /* and_imm12 -> c.andi */                                                                \
    if (do_compress() &&                                                                     \
        (Rd == Rs1 && Rd->is_compressed_valid() && is_simm(imm, 6))) {                       \
      c_andi(Rd, imm);                                                                       \
      return;                                                                                \
    }                                                                                        \
...
#define INSN(NAME)                                                                           \
  void NAME(Register Rd, int32_t imm) {                                                      \
    /* lui -> c.lui */                                                                       \
    if (do_compress() && (Rd != x0 && Rd != x2 && imm != 0 && is_simm(imm, 18))) {           \
      c_lui(Rd, imm);                                                                        \
      return;                                                                                \
    }                                                                                        \
    _lui(Rd, imm);                                                                           \
