...
void TemplateTable::resolve_cache_and_index_for_method(int byte_no,
  ...
  switch(byte_no) {
    case f1_byte:
      __ la(temp, Address(Rcache, in_bytes(ResolvedMethodEntry::bytecode1_offset())));
      break;
    case f2_byte:
      __ la(temp, Address(Rcache, in_bytes(ResolvedMethodEntry::bytecode2_offset())));
      break;
  }
...
void TemplateTable::load_resolved_method_entry_handle(Register cache,
  ...
  __ push_reg(x19);
  __ mv(x19, ref_index);
  __ load_resolved_reference_at_index(ref_index, x19);
  __ pop_reg(x19);
...
void TemplateTable::invokevirtual(int byte_no) {
  ...
  load_resolved_method_entry_virtual(x12,     // ResolvedMethodEntry*
                                     ...
                                     x13);    // flags
  prepare_invoke(x12, x13); // recv, flags
...
void TemplateTable::invokespecial(int byte_no) {
  ...
  load_resolved_method_entry_special_or_static(x12,     // ResolvedMethodEntry*
                                               xmethod, // Method*
                                               x13);    // flags
  prepare_invoke(x12, x13);  // get receiver also for null check and flags
  __ verify_oop(x12);
  __ null_check(x12);
  // do the call
  __ profile_call(x10);
...
void TemplateTable::invokeinterface(int byte_no) {
  ...
  load_resolved_method_entry_interface(x12,     // ResolvedMethodEntry*
                                       ...
                                       x13);    // flags
  prepare_invoke(x12, x13); // receiver and flags
