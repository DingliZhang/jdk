...
static int patch_offset_in_jal(address branch, int64_t offset) {
  assert(is_simm(offset, 21) && ((offset & 0x1) == 0), "offset is too large to be patched in one jal insrusction!\n");
...
static int patch_offset_in_conditional_branch(address branch, int64_t offset) {
  assert(is_simm(offset, 13) && ((offset & 0x1) == 0), "offset is too large to be patched in one beq/bge/bgeu/blt/bltu/bne insrusction!\n");
